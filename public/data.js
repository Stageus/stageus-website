
// 신민서 님, 이준규 님
const lecture = {
    "generation": 6,
    "information": [
        {
            "name": "파이썬 응용 프로그램 과정",
            "duration": "11월 15일 ~ 1월 24일 (월)",
            "number": 2
            // 한만욱 (신청완 / 문자 1,2 / 결제미완 / 세금미완)
            // 김재걸 (신청완 / 문자 1,2 / 결제미완 / 세금미완)
        },
        {
            "name": "웹 기초 과정",
            "duration": "11월 16일 ~ 1월 25일 (화)",
            "number": 2
            // 황석하 (신청완 / 문자 1,2 / 결제완 / 세금완)
            // 함윤지 (신청완 / 문자 1,2 / 결제미완 / 세금미완)
        },
        {
            "name": "웹 프론트엔드 과정",
            "duration": "11월 24일 ~ 2월 9일 (수)",
            "number": 0
        },
        {
            "name": "백엔드 서버 과정",
            "duration": "11월 25일 ~ 2월 10일 (목)",
            "number": 3
            // 강민우 (신청완 / 문자완 / 결제완 / 세금미완)
            // 김해일 (신청완 / 문자완 / 결제미완 / 세금미완)
            // 김민건 (신청완 / 문자완 / 결제미완 / 세금미완)
        },
        {
            "name": "안드로이드 앱 과정",
            "duration": "11월 26일 ~ 2월 11일 (금)",
            "number": 2
            // 김용준, 문승재 12월 31일 시작
        },
        {
            "name": "웹 기초 과정",
            "duration": "11월 21일 ~ 1월 30일 (일)",
            "number": "마감"
        }
    ]
};

const path = [
    "introduce",
    "files/curriculum.pdf",
    "faq",
    "https://open.kakao.com/o/sFu0jAwd",
    "refund"
]

const footer_img = [
    "#",
    "https://www.youtube.com/channel/UCktIF_yDLyBQEx5_MZuscfg",
    "https://www.instagram.com/stageus_cono"
]

const footer_data = [
    "(주)스테이지어스",
    "399-81-02547",
    "인천 미추홀구 인하로 55 지하 1층",
    "Choi Minseok | 최민석",
    "010. 8743. 9598",
    "cono@stageus.co.kr"
]